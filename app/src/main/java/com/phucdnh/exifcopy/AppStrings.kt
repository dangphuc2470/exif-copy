...
object Strings {
    ...
    fun showWatermarkAdjustmentTitle(isVi: Boolean) = if (isVi) "Hiển thị hộp tinh chỉnh vị trí Watermark" else "Show Watermark Adjustment Card"
    fun showWatermarkAdjustmentDesc(isVi: Boolean) = if (isVi) "Bật để hiển thị thanh tinh chỉnh tọa độ và kích thước hộp watermark ở màn hình chính. Khi TẮT, ứng dụng sẽ ẩn hộp này và luôn tự động nhận diện vị trí." else "Turn ON to show manual offset and box size adjustment on the main screen. When OFF, auto-detect is always used."
    ...
    fun resetCustomPosBtn(isVi: Boolean) = if (isVi) "Đặt lại mặc định" else "Reset default"
    fun savedCustomPosToast(isVi: Boolean) = if (isVi) "Đã lưu vị trí và kích thước tùy chỉnh!" else "Saved custom position and size!"
    fun resetCustomPosToast(isVi: Boolean) = if (isVi) "Đã đặt lại vị trí mặc định!" else "Reset to default position!"
